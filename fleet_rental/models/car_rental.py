...
class CarRentalContract(models.Model):
    ...
    _description = 'Manajemen Rental Mobil'
    ...
    image = fields.Binary(related='vehicle_id.image_128', string="Gambar Kendaraan")
    ...
    vehicle_id = fields.Many2one('fleet.vehicle', string="Kendaraan", required=True, help="Kendaraan",
                                 readonly=True,
                                 states={'draft': [('readonly', False)]}
                                 )
    car_brand = fields.Many2one('fleet.vehicle.model.brand', string="Merk Armada", size=50,
                                related='vehicle_id.model_id.brand_id', store=True, readonly=True)
    car_color = fields.Char(string="Warna Armada", size=50, related='vehicle_id.color', store=True, copy=False,
                            default='#FFFFFF', readonly=True)
    cost = fields.Float(string="Biaya Rental", help="Kolom ini untuk menentukan biaya rental", required=True)
    rent_start_date = fields.Date(string="Tanggal Mulai Kontrak", required=True, default=str(date.today()),
                                  help="Tanggal mulai kontrak", track_visibility='onchange')
    rent_end_date = fields.Date(string="Tanggal Akhir Sewa", required=True, help="End date of contract",
    ...
    cost_frequency = fields.Selection([('no', 'No'), ('daily', 'harian'), ('weekly', 'Mingguan'), ('monthly', 'Bulanan'),
                                       ('yearly', 'Tahunan')], string="Frekuensi Biaya Berulang",
                                      help='Frekuensi biaya berulang', required=True)
    ...
    attachment_ids = fields.Many2many('ir.attachment', 'car_rent_checklist_ir_attachments_rel',
                                      'rental_id', 'attachment_id', string="Lampiran",
    ...
    def fleet_scheduler1(self, rent_date):
        ...
        if product_id.property_account_income_id.id:
            ...
            raise UserError(
                _('Silakan tentukan akun pendapatan untuk produk ini: "%s" (id:%d).') % (product_id.name,
                                                                                     product_id.id))
        recurring_data = {
